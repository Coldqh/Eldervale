...
world.month = 2;
const housingGovernment = world.settlementGovernments.find(item => item.settlementId === house!.settlementId)!;
housingGovernment.treasury = Math.max(housingGovernment.treasury, 1000);
automaticHousingRequest.priority = 100;
advanceInstitutionSystem(world, new RNG('city-council-housing-smoke'));
...
assert.ok(['started', 'blocked', 'rejected'].includes(automaticHousingRequest.status), 'городская заявка должна пройти решение власти и строительную систему');
...
} else assert.ok(automaticHousingRequest.blockedReason, 'неисполненная заявка должна сохранить точную политическую или строительную причину');
advanceCitySimulation(world, new Set([house!.settlementId]));
...
const projectSettlement = world.settlements.find(item => item.id !== house!.settlementId
  && world.constructionProjects.filter(project => project.settlementId === item.id && !['завершено', 'заброшено'].includes(project.stage)).length < 3) ?? world.settlements[0]!;
const projectUrbanBefore = world.urbanStates.find(item => item.settlementId === projectSettlement.id)!;
for (const request of projectUrbanBefore.projectQueue.filter(item => ['requested', 'approved'].includes(item.status))) {
  request.status = 'blocked';
  request.nextReviewTick = Number.MAX_SAFE_INTEGER;
}
const queueBefore = projectUrbanBefore.projectQueue.length;
...
const queuedBeforeDecision = projectUrbanBefore.projectQueue.find(item => item.requestedBuildingType === 'watchtower' && item.reason.includes('единого городского API'))!;
queuedBeforeDecision.priority = 100;
const projectGovernment = world.settlementGovernments.find(item => item.settlementId === projectSettlement.id)!;
projectGovernment.treasury = Math.max(projectGovernment.treasury, 1000);
world.month = 2;
advanceInstitutionSystem(world, new RNG('city-project-council-api-smoke'));
advanceConstruction(world, new RNG('city-project-build-api-smoke'), buildWorldIndexes(world), new Set([projectSettlement.id]));
...
assert.ok(queued && ['blocked', 'started', 'rejected'].includes(queued.status), 'городской запрос должен пройти политическое решение и либо получить участок, либо сохранить причину отказа');
...
if (queued.status === 'blocked' || queued.status === 'rejected') assert.ok(queued.blockedReason, 'неисполненный проект должен хранить причину');
