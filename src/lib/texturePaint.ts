...
export function paintFeature(ctx: CanvasRenderingContext2D, feature: LocalFeature, x: number, y: number, size: number) {
  ...
  else if (feature === 'field') { ctx.strokeStyle = '#d1aa5e'; ctx.lineWidth = Math.max(1, size * .08); for (let i = 1; i <= 4; i += 1) { const xx = x + size * i / 5; ctx.beginPath(); ctx.moveTo(xx, y + size * .12); ctx.lineTo(xx, y + size * .88); ctx.stroke(); } }
...
export function paintMarker(ctx: CanvasRenderingContext2D, marker: LocalMarker, x0: number, y0: number, size: number) {
  ...
  if (marker.kind === 'person' || marker.kind === 'group') { ctx.fillStyle = '#f2dfae'; ctx.beginPath(); ctx.arc(cx, cy - r * .35, r * .45, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(cx, cy + r * .55, r * .65, Math.PI, 0); ctx.fill(); }
  else if (marker.kind === 'building' || marker.kind === 'establishment' || marker.kind === 'settlement') { ctx.fillStyle = marker.kind === 'establishment' ? '#d7a95b' : '#8e8068'; ctx.beginPath(); ctx.moveTo(cx - r, cy); ctx.lineTo(cx, cy - r); ctx.lineTo(cx + r, cy); ctx.lineTo(cx + r, cy + r); ctx.lineTo(cx - r, cy + r); ctx.closePath(); ctx.fill(); ctx.stroke(); }
