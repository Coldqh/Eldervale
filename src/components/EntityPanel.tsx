...
function renderStats(world: WorldState, ref: EntityRef, entity: any, onSelect: (ref: EntityRef) => void) {
  ...
  if (ref.kind === 'settlement') {
    ...
    const regionalEconomy = world.settlementRegionalEconomies?.find(item => item.settlementId === entity.id);
    const localDeposits = (world.resourceDeposits ?? []).filter(item => item.assignedSettlementId === entity.id);
    const tradeContracts = (world.tradeContracts ?? []).filter(item => item.fromSettlementId === entity.id || item.toSettlementId === entity.id).filter(item => ['proposed', 'active', 'suspended', 'rejected'].includes(item.status));
    const institutionDecisions = (world.institutionDecisions ?? []).filter(item => item.settlementId === entity.id).slice(-8).reverse();
    const specializationLabel: Record<string, string> = { subsistence: 'самообеспечение', agriculture: 'земледелие', mining: 'добыча', forestry: 'лесное хозяйство', fishing: 'рыбный промысел', craft: 'ремесло', trade: 'торговля', military: 'военный центр', scholarly: 'центр знаний' };
    return <>
    ...
    {row('Экспедиции основателей', expeditions.length ? <span className="relationship-stack">{expeditions.map(expedition => {
      ...
      const status = expedition.status === 'traveling' ? 'в пути' : expedition.status === 'camped' ? 'строит лагерь' : expedition.status === 'returning' ? 'возвращается' : expedition.status === 'founded' ? `основала ${getTitle(world, { kind: 'settlement', id: expedition.foundedSettlementId! })}` : expedition.status === 'returned' ? 'вернулась' : 'провалилась';
    ...
    {regionalEconomy && row('Специализация', `${specializationLabel[regionalEconomy.specialization] ?? regionalEconomy.specialization}${regionalEconomy.secondarySpecialization ? ` · вторично: ${specializationLabel[regionalEconomy.secondarySpecialization] ?? regionalEconomy.secondarySpecialization}` : ''}`)}
    {regionalEconomy && row('Ресурсная база', localDeposits.map(item => `${CIVILIZATION_CONTENT.resourceById.get(item.templateId)?.name ?? item.templateId}: ${Math.round(item.remaining)}/${Math.round(item.initialAmount)} · качество ${Math.round(item.quality)}%`).join('; ') || 'нет доступных месторождений')}
    {regionalEconomy && row('Импортозависимость', `${Math.round(regionalEconomy.importReliance)}% · рынки ${Math.round(regionalEconomy.marketAccess)}% · производство ${Math.round(regionalEconomy.productionCapacity)}%`)}
    {regionalEconomy && row('Критический импорт', regionalEconomy.criticalImportTemplateIds.map(id => CIVILIZATION_CONTENT.resourceById.get(id)?.name ?? id).join(', ') || 'нет')}
    {regionalEconomy && row('Экспортная база', regionalEconomy.exportTemplateIds.map(id => CIVILIZATION_CONTENT.resourceById.get(id)?.name ?? id).join(', ') || 'нет')}
    {regionalEconomy && row('Экономический кризис', regionalEconomy.activeCrisis ? `${regionalEconomy.activeCrisis} · ${regionalEconomy.crisisMonths} мес.` : 'нет')}
    {row('Договоры поставок', tradeContracts.map(item => `${item.status === 'active' ? 'активен' : item.status === 'proposed' ? 'на переговорах' : item.status === 'rejected' ? 'отклонён' : 'приостановлен'}: ${CIVILIZATION_CONTENT.resourceById.get(item.templateId)?.name ?? item.templateId}, ${item.fromSettlementId === entity.id ? 'экспорт' : 'импорт'} ${item.targetQuantity.toFixed(1)} ед.${item.brokerCharacterId ? ` · отвечает ${getTitle(world, { kind: 'character', id: item.brokerCharacterId })}` : ''}`).join('; ') || 'нет')}
    {row('Решения институтов', institutionDecisions.map(item => `${item.kind}: ${item.status} · ${getTitle(world, { kind: 'character', id: item.actorCharacterId })}${item.result ? ` — ${item.result}` : ''}`).join('; ') || 'нет записанных решений')}
  ...
  if (ref.kind === 'character') {
    ...
    const expedition = entity.expeditionId ? (world.settlementExpeditions ?? []).find(item => item.id === entity.expeditionId) : undefined;
    return <>
