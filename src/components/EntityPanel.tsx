...
function renderStats(world: WorldState, ref: EntityRef, entity: any, onSelect: (ref: EntityRef) => void) {
  ...
  if (ref.kind === 'settlement') {
    ...
    const tradeContracts = (world.tradeContracts ?? []).filter(item => item.fromSettlementId === entity.id || item.toSettlementId === entity.id).filter(item => item.status === 'active' || item.status === 'suspended');
    ...
    {row('Договоры поставок', tradeContracts.map(item => `${item.status === 'active' ? 'активен' : 'приостановлен'}: ${CIVILIZATION_CONTENT.resourceById.get(item.templateId)?.name ?? item.templateId}, ${item.fromSettlementId === entity.id ? 'экспорт' : 'импорт'} ${item.targetQuantity.toFixed(1)} ед.`).join('; ') || 'нет')}
