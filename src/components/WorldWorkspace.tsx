...
export type WorldView = 'map' | 'archive' | 'chronicle' | 'stories' | 'houses' | 'climate' | 'atlas' | 'local';
    ...
    <main className="world-workspace">
      {view === 'map' && <section className="map-stage workspace-view">
        <div className="map-toolbar">
          ...
          <div className="map-toolbar-right"><div className="map-legend"><span><i className="dot settlement-dot" />Поселение</span><span><i className="triangle" />Угроза</span><span><i className="army-mark" />Армия</span></div><button className="climate-entry-button" disabled={busy} onClick={() => setView('climate')}>Климат</button><button className="atlas-entry-button" disabled={busy} onClick={() => setView('atlas')}>Исторический атлас</button></div>
        </div>
