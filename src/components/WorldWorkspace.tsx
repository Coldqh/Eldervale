    ...
    <main className="world-workspace">
      {view === 'map' && <section className="map-stage workspace-view">
        <div className="map-toolbar">
          ...
          <div className="map-toolbar-right"><div className="map-legend"><span><i className="dot settlement-dot" />Поселение</span><span><i className="triangle" />Угроза</span><span><i className="army-mark" />Армия</span></div><button className="population-entry-button" disabled={busy} onClick={() => setView('population')}>Население</button><button className="population-entry-button" disabled={busy} onClick={() => setView('city')}>Города</button><button className="climate-entry-button" disabled={busy} onClick={() => setView('climate')}>Климат</button><button className="atlas-entry-button" disabled={busy} onClick={() => setView('atlas')}>Исторический атлас</button></div>
        </div>
      ...
      {view === 'city' && <CityView world={world} onSelect={onSelect} />}
